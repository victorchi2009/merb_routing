  ...
  class Router
    ...
    class Route #:nodoc:
      ...
      def initialize(conditions, params, deferred_procs, options = {})
        ...
        @conditions.delete(:method) if @conditions[:method] == 'any'
      ...
      def compiled_statement
        code = ""
        code << "if " << condition_statements.join(" && ") << "\n"
        code << "  return "

        # First, we need to always return the value of the
        # deferred block if it explicitly matched the route
        if @redirects
          code << "request.matched? ? [#{@index.inspect}, block_result] : " if @deferred_procs.any?
